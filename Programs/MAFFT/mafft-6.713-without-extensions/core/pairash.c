...
static float callrash( char **mseq1, char **mseq2, double *equiv, char *fname1, char *chain1, char *fname2, char *chain2, int alloclen )
{
	FILE *fp;
	int res;
	...
	else
	{
		...
		sprintf( com, "grep -v successful %s > _rashout", cachefile );
		system( com );
	}
...
//			fprintf( stderr, "command = %s\n", command );
	...
	*alloclenpt = nlenmax*2;
	...
	for( i=0; i<istr; i++ )
	{
		...
		callrash( *mseq1pt, *mseq2pt, *equivpt, (*strfiles)[i], (*chainids)[i], (*strfiles)[i], (*chainids)[i], *alloclenpt );
...
#endif
			{
				switch( alg )
				{
					...
					case( 'R' ):
						...
						pscore = callrash( mseq1, mseq2, equiv, strfiles[i], chainids[i], strfiles[j], chainids[j], alloclen );
						off1 = off2 = 0;
						break;
#if 0
					case( 'V' ):
